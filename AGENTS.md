# AGENTS.md — Regras do Projeto Sweet & Coffee Week
...
**Preservar a direção atual da Home.** Ela já segue a narrativa “festival em
movimento” (hero fotográfica → rotas → cidade → ciclo → números → imprensa →
realização). Quando houver solicitação de ajuste, priorizar correções de conteúdo,
dados, links, acessibilidade, responsividade e refinamentos locais. Não propor
reconstrução estrutural, novas seções ou mudança de linguagem visual sem pedido
...
**Fluxo futuro de participação.** A página Participar é a porta comercial e o
pré-cadastro de estabelecimentos. O formulário deve registrar o interesse para
análise no painel administrativo, não depender de abertura de Instagram. Após
aprovação, a organização criará uma área privada do participante para completar
informações do estabelecimento e do combo, receber avisos e acompanhar demandas
da edição. A área privada não faz parte do escopo atual; a página pública e o
modelo do formulário devem apenas preparar esse fluxo sem prometer uma função
ainda indisponível ao público.
